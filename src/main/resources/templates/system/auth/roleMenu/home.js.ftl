...
app.controller('appCtl', function($scope, $http) {
	...
	$scope.getData = function(){
		var listUrl = "${base}/roleMenu/list.do?size=" + $scope.pageSize + "&page=" + $scope.pageIndex;
	...
	$scope.optionsInit = function(){
		var optionsUrl = "${base}/role/options.do";
	    
	    $http.get(optionsUrl).success(function (response) {
	    	$scope.options.roleOptions = response;
	    });
	    
	    optionsUrl = "${base}/menu/options.do";
	    
	    $http.get(optionsUrl).success(function (response) {
	    	$scope.options.menuOptions = response;
	    });
	};
	...
	$scope.createConfirm = function(){
		$.ajax({
			cache: true,
			type: 'POST',
			url: "${base}/roleMenu/create.do",
			data: angular.toJson($scope.createItem),
			dataType:"json",
	...
	$scope.editConfirm = function(){
		...
		$.ajax({
			type: 'POST',
			url: "${base}/roleMenu/edit.do",
			data: angular.toJson($scope.editItem),
			dataType:"json",
    		contentType:'application/json;charset=UTF-8',
			error: function(req){
				$scope.editOk = false;
	...
	$scope.deleteConfirm = function(){
		...
		$.ajax({
			type: 'POST',
			url: "${base}/roleMenu/delete.do",
			data: angular.toJson($scope.deleteItem),
			dataType:"json",
    		contentType:'application/json;charset=UTF-8',
			error: function(req){
